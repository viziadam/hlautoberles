...
// Adminon gyakran jó default, ha mindent mutatunk.
// Ha itt is Pending+Reserved kell, tedd át erre:
...
const StatusFilter = ({ className, collapse, onChange }: StatusFilterProps) => {
  ...
  const allStatuses = useMemo(() => bookcarsHelper.getAllBookingStatuses(), [])

  const [checkedStatuses, setCheckedStatuses] =
    useState<bookcarsTypes.BookingStatus[]>(DEFAULT_STATUSES)

  const allChecked =
    checkedStatuses.length > 0 && checkedStatuses.length === allStatuses.length
  ...
  // ha szeretnéd, hogy betöltéskor azonnal alkalmazza a default szűrőt
  useEffect(() => {
          ...
          {statuses.map((status) => {
            ...
            return (
              <li key={status.value}>
                ...
                />
                <BookingStatus value={status.value} onClick={() => toggleStatus(status.value)} />
              </li>
            )
          })}
        </ul>
