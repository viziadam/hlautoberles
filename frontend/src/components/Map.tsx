...
const ZoomControlledLayer = ({ zoom, minZoom, children }: ZoomControlledLayerProps) => {
  if (zoom >= minZoom) return <>{children}</>
  return null
}

/**
 * 1) Egyszeru "fitBounds": automatikusan rakozelit a locations pontokra.
...
const Map = ({
  ...
  const getLocationMarkers = (): MarkerItem[] =>
    (locations
      && locations
        .filter((l) => l.latitude && l.longitude)
        .map((l) => ({ name: l.name!, position: new L.LatLng(l.latitude!, l.longitude!) }))) || []

  // kiszedjuk egyszer, hogy:
  // - FitToMarkers kapja meg (fitBounds)
  // - render is ugyanazt hasznalja
  const locationMarkers = getLocationMarkers()

  /**
   * 2) Kiemeles: CircleMarker halo a Marker mogott
   * - highlight=true esetén rajzolja a narancs korongot
   */
  const getMarkers = (__markers: MarkerItem[], highlight = false) =>
    __markers.map((marker) => (
  ...
  return (
    <>
      ...
      >
        ...
        <ZoomControlledLayer zoom={zoom} minZoom={_initialZoom}>
          {getMarkers(locationMarkers, true)}   {/* <-- highlight ON */}
          {getParkingSpots()}
        </ZoomControlledLayer>
